...
import { BookA, BookTemplate } from "lucide-react";
...
const MyBorrowedBooks = () => {
  ...
  return <>
  <main className="relative flex-1 p6 pt-28">
    <Header/>
  ...
  Borrowed Books
    ...
    <header className="flex flex-col gap-3 sm:flex-row md:items-center">
     ...
     className={`text-center border-2 font-semibold py-2 w-full sm:w-72  relative rounded sm:rounded-tr-none
      sm:rounded-br-none sm:rounded-tl-lg sm:rounded-bl-lg
     ...
     className={`text-center border-2 font-semibold py-2 w-full sm:w-72  relative rounded sm:rounded-tl-none
      sm:rounded-bl-none sm:rounded-tr-lg sm:rounded-br-lg
        ...
        <tbody>
          {
            booksToDisplay&& booksToDisplay.map((book,index)=>(
              ...
              className={(index+1)%2===0?"bg-gray-50":""}>
                ...
                <td className="px-4 py-2"><BookA onClick={()=>openReadPopup(book._id)}/> </td>
